...
def main():
    ...
    if not doc_files:
        console.print("[yellow][WARN] No documents found in documents/ folder[/yellow]")
        return
    ...
    for doc_file in doc_files:
        try:
            ...
            console.print(f"[green][OK] Loaded {doc_file.name} - {result['num_chunks']} chunks[/green]")
            loaded += 1
        except Exception as e:
            console.print(f"[red][X] Failed to load {doc_file.name}: {str(e)}[/red]")
            ...
            try:
                ...
                console.print(f"[green][OK] Loaded as text - {result['num_chunks']} chunks[/green]")
                loaded += 1
            except Exception as e2:
                console.print(f"[red][X] Complete failure: {str(e2)}[/red]")
                failed += 1
    
    console.print(f"\n[bold]Summary:[/bold]")
    console.print(f"  [OK] Loaded: {loaded}")
    console.print(f"  [X] Failed: {failed}")
    
    if loaded > 0:
        console.print("\n[yellow]Building vector index...[/yellow]")
        engine.build_index()
        console.print("[green][OK] Index built successfully![/green]")
        
        # Save the engine
        console.print("\n[yellow]Saving RAG engine...[/yellow]")
        engine.save()
        console.print("[green][OK] Engine saved![/green]")
        ...
        console.print("\n[green][OK] All done! You can now run:[/green]")
